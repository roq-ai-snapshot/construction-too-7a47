...
type ColumnType = ColumnDef<RentalRequestInterface, unknown>;

interface RentalRequestListPageProps {
...
export function RentalRequestListPage(props: RentalRequestListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RentalRequestInterface>>(
    () => `/rental-requests?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RentalRequestInterface) => {
    if (hasAccess('rental_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/rental-requests/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/rental-requests/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="rental_request" />
        </Flex>
